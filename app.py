...
class MarkdownPDFExtractor(PDFExtractor):
    ...
    def extract(self, output_dir):
        try:
            ...
            markdown_content, markdown_pages = self.extract_markdown(output_dir)
            self.save_markdown(markdown_content, output_dir)
            self.logger.info(
                f"Markdown content has been saved to {Path(output_dir)}/{self.pdf_filename}.md"
            )
    ...
    def extract_markdown(self, output_dir):
                ...
                if len(page.get_images()) > 0 and len(page.get_images()) <= 128:
                    for block in blocks:
                        if block["type"] == 0:  # Text
                            ...
                            page_content += self.process_image_block(page, block, output_dir)
    ...
    def process_image_block(self, page, block, output_dir):
        ...
        try:
            ...
            image_path = (
                ...
                Path(output_dir) / image_filename
    ...
    def save_markdown(self, markdown_content, output_dir):
        """Save the markdown content to a file."""
        try:
            # os.makedirs(Path(config["OUTPUT_DIR"]), exist_ok=True)
            os.makedirs(Path(output_dir), exist_ok=True)
            with open(
                f"{Path(output_dir)}/{self.pdf_filename}.md",
                "w",
                encoding="utf-8",
            ) as f:
...
def process_file(fileobj):
    path = os.path.abspath(fileobj)
    basename = os.path.basename(fileobj)
    curr_dt = datetime.now()
    timestamp = int(round(curr_dt.timestamp()))
    # print("process_file", path)
    file_stats = os.stat(path)
    # print("File Stats", file_stats)
    Path("./upload/" + str(timestamp)).mkdir(parents=True, exist_ok=True)
    path2 = "./upload/" + str(timestamp) + "/" + str(basename)
    # print("File:", path2)
    shutil.copyfile(path, path2)

    extractor = MarkdownPDFExtractor(path2)
    markdown_pages = extractor.extract("output/" + str(timestamp))
    print ("########################")
    print (markdown_pages)
    print ("########################")
    zipfile = shutil.make_archive("output/" + str(timestamp), 'zip', "output/" + str(timestamp))
    # return path2
    print (zipfile)
    return zipfile
...
desc = """PDF to markdown provides a simple and efficient way to convert PDF files into markdown format. With this tool, users can upload a PDF file, and the app processes it to extract content such as text and basic formatting. The resulting markdown file is then packaged into a zip file for easy download including the markdown file and the images."""
long_desc = """Perfect for integrating PDFs into your markdown workflows, this app saves time and ensures a smooth content conversion process!"""
